...
public class Controller implements Api {
    ...
    public ModelAndView newBook() {
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("/"); // указываю какую страницу вернуть
        return modelAndView;
    }

    @Override
    public void addBook(BookDTO bookDTO, MultipartFile file, HttpServletResponse response) throws IOException, InterruptedException {
        bookDTO.setId(UUID.fromString(UUID.randomUUID().toString()));
        bookService.createBook(bookDTO, getCurrentUser(), getCurrentRecord());
        response.sendRedirect("/");
    }

    public Record getCurrentRecord() {
        return (Record) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    }
